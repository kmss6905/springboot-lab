...
public class GithubController {
    ...
    @GetMapping("/complex/{repo}")
    public Mono<?> getComplexApi(@PathVariable String repo){

        Mono<?> repoMono = githubClient.getGithubRepositoryByJsonNode(appProperties.getGithub().getUsername(), repo);
        Mono<?> repoMono2 = githubClient.getGithubRepositoryByJsonNode(appProperties.getGithub().getUsername(), repo);

        return Mono.zip(repoMono, repoMono2);
    }
